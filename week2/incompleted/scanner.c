...
Token* readIdentKeyword(void) {
  ...
  while ((currentChar != EOF) && ((charCodes[currentChar] == CHAR_LETTER) || currentChar == 95)) { // gap ki tu ket thuc file hoac chu cai
...
Token* readNumber(void) {
  ...
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT)) { //gap ki tu ket thuc file hoac gap chu so
    if(count < 5)token->string[count++] = (char)currentChar;
    readChar();
  }
  if(count > 4){
    printf("%d-%d: ERROR !!! YOUR NUMBER IS TOO LONG\n",token->lineNo, token->colNo);
    return token;
  }
...
void printToken(Token *token) {
  ...
  case SB_MO: printf("SB_MO\n"); break;
  case SB_DONG: printf("SB_DONG\n"); break;
  }
}
