...
void compileConstDecls(void) {  //bien dich khai bao hang
  Object* constObj;
...
void compileWhileSt(void) {
  ...
  eat(KW_WHILE);
  compileCondition();  //bien dich dieu kien

  fjInstruction =genFJ(DC_VALUE);  //genFJ :fail jump
...
void compileForSt(void) {
  ...
  genCV();  //xu ly cho nay : genCV (copy top)
  ...
  genST();  //store
  genCV();
  genLI();  //load indirect(cai dat gian tiep)
  ...
  genJ(beginLoop);
  updateFJ(fjInstruction ,getCurrentCodeAddress());
  genDCT(1);  //decrement t(giam di mot luong)
}
...
Type* compileExpression(void) {
  // TODO: generate code for expression (tao ra code cho bieu thuc)
  Type* type;
  ...
  case SB_MINUS:
    ...
    genNEG();  // gap dau - thi moi phai xu ly ,con lai co ve khong can
...
Type* compileFactor(void) {  //example :ident (hoac :10 ;a ;function vapa rameter trong phan dinh danh(ident))
  ...
  case TK_NUMBER:
    ...
    genLC(currentToken->value);  //xu ly load constant
  ...
  case TK_CHAR:
    ...
    genLC(currentToken->value);  //xu ly o day
  ...
  genLC(obj->constAttrs->value->intValue);  //xu ly kieu int

	break;
      case TP_CHAR:
	type = charType;

  genLC(obj->constAttrs->value->charValue);  //xu ly kieu char
	...
	//genHL(); //halt :tam dung
  genLI();   //xu ly load indirect :chay gian tiep

      } else {
	type = obj->varAttrs->type;

  genVariableValue(obj);   //xu ly gia tri
...
Type* compileIndexes(Type* arrayType) {
  // TEMPORARY: halt  (temporary :tam thoi ;halt :tam dung lai)
  Type* type;
