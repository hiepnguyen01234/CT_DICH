...
int sizeOfType(Type* type) {
  ...
  switch(type->typeClass){
    ...
    case TP_CHAR :
      ...
      break;
    case TP_ARRAY :
      size =type->arraySize *(sizeOfType(type->elementType)); //cap nhat de quy cap phat bo nho cho cac phan tu cua mang
      return size;
      break;
    default :
      break;
...
void declareObject(Object* obj) {
  ...
  if(symtab->currentScope ==NULL){
    addObject(&(symtab->globalObjectList),obj);  // them doi tuong toan cuc neu symtab->currentScope rong
  }
  else{
    switch(obj->kind){
      case OBJ_VARIABLE :
        ...
        symtab->currentScope->frameSize +=sizeOfType(obj->varAttrs->type);  // neu la bien thi kich co tang them tuy bien
        break;
      case OBJ_PARAMETER :
        obj->paramAttrs->scope =symtab->currentScope;
        obj->paramAttrs->localOffset =symtab->currentScope->frameSize;
        symtab->currentScope->frameSize ++;       // kich co tang len 1
        ...
        break;
      case OBJ_FUNCTION :
        obj->funcAttrs->scope->outer =symtab->currentScope;  // vung ngoai =vung hien tai
        break;
      case OBJ_PROCEDURE :
        obj->procAttrs->scope->outer =symtab->currentScope;  // vung ngoai =vung hien tai
        break;
      default :
        break;
      }
