...
void compileConstDecls(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      eat(TK_IDENT);
      checkFreshIdent(currentToken->string);  //kiem tra xem da khai bao chua
...
void compileBlock(void) {
  ...
  jmp = genJ(DC_VALUE);  //nhay den dia chi DC
  ...
  updateJ(jmp,getCurrentCodeAddress());  //jmp->q = dia chi code hien tai
...
void compileSubDecls(void) {  //bien dich chuong trinh con hoac ham
...
void compileParams(void) {
  ...
  //bat dau sua(tam xoa roi copy lai o tren)
    eat(SB_LPAR);
    compileParam();
    while (lookAhead->tokenType == SB_SEMICOLON) {
      eat(SB_SEMICOLON);
...
void compileForSt(void) {
  ...
  genST();  //luu tru
  genCV();
  genLI();  //load thu vien(load inderect)
...
Type* compileFactor(void) {
  ...
  case TK_NUMBER:
    eat(TK_NUMBER);
    type = intType;
    genLC(currentToken->value);  //load hang so
    break;
  case TK_CHAR:
    eat(TK_CHAR);
    type = charType;
