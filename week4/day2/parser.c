...
void compileProgram(void) {
  ...
  eat(SB_PERIOD);  // kw_program ->tk_ident ->create_object(name ,kind ,attribute ,scope) ->sb_semicolon ->enter_scope ->
                    // compile(kw_const,type,var,...) ->exit
}
...
void compileBlock(void) {
  ...
  else compileBlock2();  // kw_const ->while(look->ahead ==tk_indent) {tk_ident ->create_object ->sb_eq ->
                          // compile_attrs ->sb_semicolon ->declare_obj} ->compile_block2
}
...
void compileBlock4(void) {
  compileSubDecls();  //compile_funcdecl or compile_procdecl
  compileBlock5();
}

void compileBlock5(void) {
  eat(KW_BEGIN);
  compileStatements();  //compile for{...} or call... or while ...
  eat(KW_END);
}
