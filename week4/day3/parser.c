...
void compileBlock(void) {  // example :const ident =a ; ->compile_block2
  Object* constObj;
...
void compileBlock2(void) {   //  example :type ident =int ; ->compile_block3
  Object* typeObj;
  Type* actualType;
...
void compileBlock3(void) {  //  example :var ident :int ; ->compile_block4
  Object* varObj;
  Type* varType;
...
void compileBlock4(void) {
  compileSubDecls();  // compile function or procedure ->compile_block5
  compileBlock5();
}

void compileBlock5(void) {  // example :begin for/while/group{} end
  eat(KW_BEGIN);
  compileStatements();
  eat(KW_END);  
}
...
void compileFuncDecl(void) {  //  example :function ident( compile_param) :int ; ->compile_block -> ;
  Object* funcObj;
  Type* returnType;
...
void compileProcDecl(void) {  //  example :procedure ident( compile_param) ; ->compile_block -> ;
...
ConstantValue* compileUnsignedConstant(void) {  // make constant_tk_number /tk_ident /tk_char
  ConstantValue* constValue;
  Object* obj;
...
ConstantValue* compileConstant(void) {  // example :+ ident 
  ConstantValue* constValue;
...
void compileStatement(void) {  //  compile call/group/while/...
  switch (lookAhead->tokenType) {  
  case TK_IDENT:
    compileAssignSt();
    break;
...
void compileArguments(void) {  //  to check symbol(+,-,=,...)
  switch (lookAhead->tokenType) {  
  case SB_LPAR:
    eat(SB_LPAR);
    compileArgument();
  ...
  case KW_THEN:
    break;
  default:
    error(ERR_INVALID_ARGUMENTS, lookAhead->lineNo, lookAhead->colNo);  // example :(a,b)/=/+ 
  }
}

void compileCondition(void) {  // example :(a +b ) >= c ;
  compileExpression();
...
void compileExpression(void) {  // example :+ (a+b)
  switch (lookAhead->tokenType) {
  case SB_PLUS:
    eat(SB_PLUS);
    compileExpression2();
...
void compileExpression2(void) {   // example :(a +b ) =d ;
  compileTerm();
  compileExpression3(); 
}
...
void compileTerm(void) {  // example :a +b +c
  compileFactor();
  compileTerm2();  
}
...
void compileFactor(void) {
  ...
  case TK_IDENT:
    ...
    break;
  default:
    error(ERR_INVALID_FACTOR, lookAhead->lineNo, lookAhead->colNo);  //check obj is number/char/ident
  }
}

void compileIndexes(void) {  // example :(. a+b != c .)
...
// factor :ky tu(hoac so) ,term :bieu thuc nhan chia ,expresion :bieu thuc

// bien dich cau truc chuong trinh va khoi tao cac doi tuong ,ham ,chuong trinh con ,...