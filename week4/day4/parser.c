...
ConstantValue* compileConstant2(void) {
  ...
  case TK_IDENT:
    ...
    if (obj->constAttrs->value->type == TP_INT)
      constValue = duplicateConstantValue(obj->constAttrs->value);   // duplicate :ban sao
    else
...
void compileArgument(Object* param) {  // example :+(a*b)   // argument :tranh luan
...
void compileArguments(ObjectNode* paramList) {  // example :(a,b+c);
  //doi so
  ...
  case SB_LPAR:
    ...
    if (node==NULL)
      ...
      // sua o day
       ...
       error(nodenull,currentToken->lineNo,currentToken->colNo);
      // ket thuc sua
  ...
  //error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY,currentToken->lineNo,currentToken->colNo);
    ...
    if (node!=NULL)
      error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY,currentToken->lineNo,currentToken->colNo);  // inconsistency :mau thuan
...
void compileCondition(void) {
  ...
  checkTypeEquality(type1,type2);  // check consistency :kiem tra tinh nhat quan
}
...
Type* compileExpression3(Type *argType1) {  // example :+(a*b);
  Type* argType2;
  Type* type;
...
Type* compileTerm2(Type *argType1){  // example :*a+b)
   // TODO: check type of term2
  Type* type;
  Type *argType2;
  switch (lookAhead->tokenType) {
...
Type* compileFactor(void) {  // example :a[i]
  // TODO: parse a factor and return the factor's type

  // compile factor :bien dich he so
  Object* obj;
  Type* type;
...
Type* compileIndexes(Type* arrayType) {  // example :(.10.)
  //danh muc cua mang
